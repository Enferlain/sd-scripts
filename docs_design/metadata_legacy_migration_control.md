...
| `library/utils/safetensors_utils.py::load_metadata_from_safetensors` | Reads artifact metadata from safetensors files. | Storage / IO boundary. | Keep format-focused beside the existing safetensors reader; it is not an internal metadata source of truth. |
...
| Implementation-version lookup, thumbnail data-URL conversion, and safetensors metadata reads | Placed retained boundaries | Active checkpoints reuse the run's accepted Git revision; thumbnail conversion is private model-builder input handling; safetensors header reads live in `library/utils/safetensors_utils.py`. | The misleading `library/utils/model_metadata.py` remainder was removed; source metadata ingestion remains separately owned by `sd-scripts-8ck`. |
