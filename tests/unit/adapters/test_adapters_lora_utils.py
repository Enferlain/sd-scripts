...
from library.adapters.methods.peft.lora import lora_utils
...
class TestLoraUtils:
    ...
    def test_filter_lora_state_dict_combined(self):
        ...
        filtered = lora_utils.filter_lora_state_dict(state_dict, include_pattern="unet", exclude_pattern="block_2")
        assert len(filtered) == 1
        assert "lora_unet_block_1" in filtered

    # === load_safetensors_with_lora_and_fp8 Tests ===

    @patch("library.adapters.methods.peft.lora.lora_utils.load_safetensors_with_fp8_optimization_and_hook")
    def test_load_lora_merge_logic_linear(self, mock_load):
        ...
        def mock_loader_side_effect(
            model_files, fp8_optimization, calc_device, move_to_device, dit_weight_dtype, target_keys, exclude_keys, weight_hook
        ):
    ...
    def test_load_lora_conv2d_1x1(self, mock_load):
        ...
        result_sd = lora_utils.load_safetensors_with_lora_and_fp8(
            model_files=["dummy"], lora_weights_list=[lora_weights], lora_multipliers=[1.0], fp8_optimization=False, calc_device=calc_device
        )
    ...
    @patch("library.adapters.methods.peft.lora.lora_utils.load_safetensors_with_fp8_optimization_and_hook")
    @patch("library.adapters.methods.peft.lora.lora_utils.logger")
