...
import torch
from library.models import runtime_utils
...
class TestModelPrep(unittest.TestCase):
    def test_set_padding_mode_for_vae_conv2d_modules(self):
        ...
        runtime_utils.set_padding_mode_for_vae_conv2d_modules(vae, "reflect")
    ...
    @patch("library.models.runtime_utils.logger")
    def test_replace_unet_modules_mem_eff(self, mock_logger):
        # Mock UNet
        mock_unet = MagicMock()

        # Test memory efficient attention
        runtime_utils.replace_unet_modules(mock_unet, mem_eff_attn=True, xformers=False, sdpa=False)
    ...
    def test_replace_unet_modules_sdpa(self, mock_logger):
        ...
        # Test SDPA
        runtime_utils.replace_unet_modules(mock_unet, mem_eff_attn=False, xformers=False, sdpa=True)
    ...
    def test_replace_unet_modules_xformers(self, mock_logger):
        ...
        with patch.dict("sys.modules", {"xformers.ops": MagicMock()}):
            runtime_utils.replace_unet_modules(mock_unet, mem_eff_attn=False, xformers=True, sdpa=False)
    ...
    def test_patch_accelerator_for_fp16_training(self, mock_logger):
        ...
        runtime_utils.patch_accelerator_for_fp16_training(mock_accelerator)
