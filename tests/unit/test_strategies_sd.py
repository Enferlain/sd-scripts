...
class TestSdTokenizeStrategy:
    ...
    def test_tokenize_with_weights(self, mock_load_tokenizer, mock_clip_tokenizer):
        ...
        with patch.object(strategy, '_get_input_ids') as mock_get_ids:
            ...
            mock_get_ids.assert_called_with(
                mock_clip_tokenizer, "(emphasized:1.5)", 77, weighted=True
            )
...
class TestSdTextEncodingStrategy:
    ...
    def test_encode_tokens_basic(self, mock_load_tokenizer, mock_clip_tokenizer, mock_clip_text_encoder):
        ...
        result = encoding_strategy.encode_tokens(
            tokenize_strategy, [mock_clip_text_encoder], tokens
        )
    ...
    @patch.object(SdTokenizeStrategy, '_load_tokenizer')
    def test_encode_tokens_with_weights_applies_weights(
        self, mock_load_tokenizer, mock_clip_tokenizer, mock_clip_text_encoder
    ):
        ...
        result = encoding_strategy.encode_tokens_with_weights(
            tokenize_strategy, [mock_clip_text_encoder], tokens, weights
        )
...
class TestSdSdxlLatentsCachingStrategy:
    ...
    def test_init_sd_suffix(self):
        ...
        strategy = SdSdxlLatentsCachingStrategy(
            sd=True, cache_to_disk=True, batch_size=1, 
    ...
    def test_init_sdxl_suffix(self):
        ...
        strategy = SdSdxlLatentsCachingStrategy(
            sd=False, cache_to_disk=True, batch_size=1,
            skip_disk_cache_validity_check=False
        )
    ...
    def test_is_disk_cached_latents_expected_delegates(self, tmp_path):
        ...
        result = strategy.is_disk_cached_latents_expected(
            (512, 512), npz_path, flip_aug=False, alpha_mask=False
        )
    ...
    def test_cache_batch_latents_calls_vae(self):
        ...
        strategy = SdSdxlLatentsCachingStrategy(
            sd=True, cache_to_disk=False, batch_size=1,
            skip_disk_cache_validity_check=False
        )
        ...
        mock_vae.encode.return_value = Mock(
            latent_dist=Mock(sample=Mock(return_value=torch.randn(1, 4, 64, 64)))
        )
        ...
        with patch('library.strategies.strategy_sd.clean_memory_on_device'):
            with patch.object(strategy, '_default_cache_batch_latents') as mock_cache:
                strategy.cache_batch_latents(
                    mock_vae, [mock_info], flip_aug=False, 
                    alpha_mask=False, random_crop=False
                )
