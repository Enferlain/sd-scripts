...
- [ ] 6.1 Move benchmark report resource input from canonical JSONL parsing to the resource-run view.
- [ ] 6.2 Preserve existing report resource summaries and per-device/debug output during migration.
- [ ] 6.3 Implement metadata-owned projections for JSONL, report, profile, and accounting export schemas, then register produced artifacts with links to their source run.
