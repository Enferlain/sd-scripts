...
def setup_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "dst_img_folder", type=str, help="Destination folder to save the resized images / リサイズ後の画像を保存するフォルダ"
    )
    parser.add_argument(
        "--max_resolution",
        type=str,
    ...
    parser.add_argument(
        "--interpolation",
        type=str,
        choices=["area", "cubic", "lanczos4", "nearest", "linear", "box"],
        default=None,
        help="Interpolation method for resizing. Default to area if smaller, lanczos if larger / サイズ変更の補間方法。小さい場合はデフォルトでエリア、大きい場合はランチョスになります。",
    )
