...
def build_sd3_attention_masks(
    ...
    l_attn_mask = (l_tokens != tokenizer_l.pad_token_id).long()
    g_attn_mask = (g_tokens != tokenizer_g.pad_token_id).long()
    t5_attn_mask = (t5_tokens != tokenizer_t5.pad_token_id).long()
    return [l_attn_mask, g_attn_mask, t5_attn_mask]
...
def drop_cached_sd3_text_encoder_outputs(
    ...
    l_attn_mask: torch.Tensor | None,
    g_attn_mask: torch.Tensor | None,
...
class Sd3TextEncodingStrategy(TextEncodingStrategy):
    ...
    def encode_tokens_with_weights(
        ...
        """SD3 upstream does not define weighted prompt encoding in the current port."""
        del models, tokens, weights
    ...
    def encode_te_outputs_in_memory(
        ...
        outputs = {
            "lg_out": lg_out.squeeze(0).cpu(),
            "lg_pooled": lg_pooled.squeeze(0).cpu(),
            "clip_l_attn_mask": l_attn_mask.squeeze(0).cpu(),
            "clip_g_attn_mask": g_attn_mask.squeeze(0).cpu(),
            "t5_attn_mask": t5_attn_mask.squeeze(0).cpu(),
        }
        if t5_out is not None:
            outputs["t5_out"] = t5_out.squeeze(0).cpu()
        return outputs
    ...
    def concat_encodings(
        ...
        return concat_sd3_encodings(lg_out, t5_out, lg_pooled)
