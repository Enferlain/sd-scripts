...
class LossAwareTimestepSampler:
    ...
    def update(self, timesteps: torch.Tensor, per_sample_losses: torch.Tensor):
        ...
        bin_loss.index_add_(0, bins, per_sample_losses.detach())
        bin_cnt.index_add_(0, bins, torch.ones_like(per_sample_losses))
        mask = bin_cnt > 0
    ...
    def _cosine_anneal(self, step: int, total: int, start: float = 0.85, end: float = 0.35):
    ...
    def sample(self, bsz: int, device, global_step: int, max_steps: int,
        ...
        mix_p = self._cosine_anneal(global_step, max_steps, start=0.85, end=0.35)
