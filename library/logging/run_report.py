...
from library.training.trainer_utils import summarize_component_trainability
...
def _estimate_component_memory_rows(trainer: Any) -> list[dict[str, Any]]:
    try:
        components, aliases = trainer.mode.get_diagnostics_components(trainer)
    except Exception:
        return []
    ...
    for name, module in components:
        ...
        trainable_bytes = sum(p.numel() * p.element_size() for p in module.parameters() if p.requires_grad)
        rows.append(
            {
                "name": name,
                ...
                "gradient_mb_est": trainable_bytes / (1024 * 1024),
                "optimizer_state_mb_est": (trainable_bytes * optimizer_state_multiplier) / (1024 * 1024),
            }
        )
    return rows
...
def _collect_key_config_rows(trainer: Any, *, hydra_config_name: str | None, hydra_overrides: list[str]) -> list[tuple[str, Any]]:
    key_paths = [
        ...
        ("peft.method", _safe_get(trainer.cfg, "peft.method")),
