...
class GaussianMidSNRAdaptiveSampler:
    ...
    This sampler prioritizes learning from timesteps with mid-range signal-to-noise ratios (SNR),
    guided by a Gaussian prior. It adaptively adjusts sampling probabilities based on loss history.
    """
    ...
    def __init__(
        ...
        """
        Initialize the GaussianMidSNRAdaptiveSampler.
        ...
        Args:
            ...
            prior_mu (float): Mean of the Gaussian prior in log-SNR space.
            prior_sigma (float): Standard deviation of the Gaussian prior in log-SNR space.
            prior_weight (float): Weight given to the prior distribution relative to the learned loss distribution.
            warmup_steps (int): Number of steps to use the prior exclusively before mixing in loss-based sampling.
        """
        ...
        self.min_prob, self.prior_weight, self.warmup_steps = (
            float(min_prob),
            float(prior_weight),
            int(warmup_steps),
        )
    ...
    def _gaussian_prior_logits(self) -> torch.Tensor:
        """Calculate logits for the Gaussian prior."""
    ...
    def _update_prior_logits(self):
        """Update the stored prior logits."""
