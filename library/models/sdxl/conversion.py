...
def convert_sdxl_text_encoder_2_checkpoint(checkpoint, max_length):
    ...
    # Basically the same as SD2. Since logit_scale is used later, it is returned additionally.
    # logit_scale is used when saving the checkpoint.
    def convert_key(key):
        # common conversion
    ...
    # logit_scale is not included in Diffusers, but we want to restore it when saving, so we return it separately.
...
def convert_unet_state_dict(src_sd, conversion_map):
    ...
    for src_key, value in src_sd.items():
        # Searching for prefix while trimming elements from the right because iterating over everything takes too much time.
