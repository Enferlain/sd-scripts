        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                if p.dtype in {torch.float16, torch.bfloat16}:
                    ...
                    max_ema_squared = state["max_ema_squared"].to(torch.float32)
...
class FCompassADOPT(BaseOptimizer):
    ...
    def step(self, closure: Closure = None) -> Loss:
        ...
        for group in self.param_groups:
            ...
            current_beta2 = self.debias_beta(beta2, group["step"]) if group["debias_beta2"] else beta2
...
class FCompassPlus(BaseOptimizer):
    r"""Fisher Compass: Utilizing approximate fisher information to accelerate training. (Applied onto Compass)."""
    ...
    def step(self, closure: Closure = None) -> Loss:
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                momentum.mul_(beta1).add_(grad_nat, alpha=1 - beta1)
                if self.use_pnm:
                    noise_norm = math.sqrt((1.0 + self.pnm_beta) ** 2 + self.pnm_beta**2)
                ...
                if self.amsgrad:
                    ...
                    denom = (ema_squared.sqrt() / bias_correction_sqrt).add_(curr_eps)
                else:
                    ema_squared.mul_(beta2).addcmul_(full_step, full_step, value=1 - beta2)
                    denom = (ema_squared.sqrt() / bias_correction_sqrt).add_(curr_eps)

                if self.use_softplus:
                    denom = softplus(denom, beta=self.beta_softplus, threshold=self.threshold_softplus if self.threshold_softplus != 0 else curr_eps)
        ...
        if self.lookahead_step >= self.lookahead_merge_time:
            ...
            for group in self.param_groups:
                for p in group["params"]:
                    ...
                    p_fp32.mul_(self.lookahead_blending_alpha).add_(lookahead_params, alpha=1.0 - self.lookahead_blending_alpha)
                    if p.dtype in {torch.float16, torch.bfloat16}:
                        copy_stochastic_(p, p_fp32)
