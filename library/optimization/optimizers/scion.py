...
class SpectralConv(Norm):
    ...
    def lmo(self, grad: torch.Tensor, eps: float = 1e-8) -> torch.Tensor:
        ...
        grad_matrix = grad.view(len(grad), -1)
        grad_matrix = zero_power_via_newton_schulz_5(grad_matrix, self.num_steps)
        grad = grad_matrix.view(grad.shape)
    ...
    def init(self):
        for group in self.param_groups:
            ...
            for parameter in group["params"]:
                norm.init(parameter)
                parameter.mul_(group["scale"])
                state = self.state[parameter]
                state["d"] = torch.zeros_like(parameter)

                if group["use_focus"]:
                    state["pbar"] = torch.zeros_like(parameter)
    ...
    def step(self, closure: Closure = None) -> Loss:
        ...
        for group in self.param_groups:
            ...
            for parameter in group["params"]:
                ...
                if use_orthograd:
                    paper_orthograd(parameter_fp32, grad)
                ...
                if use_focus:
                    ...
                    pbar.mul_(focus_beta).add_(parameter_fp32, alpha=1.0 - focus_beta)

                update = norm.lmo(d).mul_(group["scale"])

                if not use_focus:
                    if group["constraint"]:
                        parameter_fp32.mul_(1.0 - group["lr"])
                ...
                if use_focus:
                    ...
                    if group["weight_decay"] > 0.0:
                        parameter_fp32.add_(pbar_hat, alpha=-group["lr"] * group["weight_decay"])

                    update = (parameter_fp32 - pbar_hat).sign_().mul_(0.1).add_(torch.sign(d))

                parameter_fp32.add_(update, alpha=-group["lr"])
