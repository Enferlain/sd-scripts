...
from library.optimization.optimizers.utils import copy_stochastic_, zero_power_via_newton_schulz_6
...
class GOODDOG(Optimizer):
    ...
    Utilizing a short and long-term denominator to flatten the landscape, along with magnitude-decoupled sign-based
    momentum. We also make use of atan2 for further scale invariance to stabilize potentially sharp updates.
    ...
    Arguments:
        ...
        betas (float, float, float):
            ...
            stage2 (long-term) squared running average (default: 0.9, 0.96, 0.9999999)
        weight_decay (float):
        ...
        invariant (bool):
            Scale the latent into -1 to 1 space via .arctan().sin(), then later divide by the original grad's
            .arctan().cos(). Its been tested a bit, with the general result of speeding up descent. (default: False).
        adaptive_muon (bool):
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                # Orthograd
                if group["orthograd"] and p_fp32.data.nelement() > 1:
