...
from library.optimization.optimizers.utils.galore import GaLoreProjector
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                if "rank" in group and p.dim() == 2:
                    ...
                    reconstructed_norm_grad = state["projector"].project_back(norm_grad)

                    norm_dim: int = 0 if reconstructed_norm_grad.shape[0] < reconstructed_norm_grad.shape[1] else 1

                    scaling_factor = torch.norm(reconstructed_norm_grad, dim=norm_dim) / (torch.norm(sub_grad, dim=norm_dim) + 1e-8)
                    if norm_dim == 1:
                        scaling_factor = scaling_factor.unsqueeze(1)

                    scaling_grad = full_rank_grad.sub(sub_grad).mul_(scaling_factor)
                    ...
                    norm_grad = reconstructed_norm_grad.add_(scaling_grad)
