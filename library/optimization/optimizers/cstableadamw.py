...
from library.optimization.optimizers.utils import stable_spam_clipping_compile_wrapper, stable_spam_clipping_impl
...
class CStableAdamW(BaseOptimizer):
    ...
    def reset(self):
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                state["kahan_comp"] = torch.zeros_like(p) if group["kahan_sum"] and p.dtype in {torch.float16, torch.bfloat16} else None
    ...
    def _stable_spam_clipping(
        ...
        if torch_compile:
            return stable_spam_clipping_compile_wrapper(
                state,
                grad,
                step=group_step,
                scale=ssc_scale,
        ...
        return stable_spam_clipping_impl(
            state,
            grad,
            step=group_step,
            scale=ssc_scale,
    ...
    def step(self, closure: Closure = None) -> Loss:
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                grad_for_processing = grad.clone() if use_ssc else grad
                ...
                if use_adopt:
                    ...
                    exp_avg.lerp_(grad_for_processing, weight=beta1_comp)
                ...
                if not use_atan2:
                    ...
                    if cautious_factor < 1.0:
                        ...
                        scale_cautious = torch.where(
                            align_mask,
                            torch.ones_like(grad_for_processing),
                            grad_for_processing.new_full((), cautious_factor, device=p.device, dtype=p.dtype),
                        )
