            ...
            for p in group["params"]:
                ...
                if not self.rectify_variance or step_size > 0 or n_sma >= self.n_sma_threshold:
                    if weight_decouple:
                        ...
                        p_fp32.mul_(
                            1.0
                            - (1.0 if fixed_decay else step_size if not lr_decouple else step_size / max_lr) * weight_decay * swd_scaling
                        )
...
class CompassPlus(BaseOptimizer):
    ...
    def __init__(
        ...
        slow_ema_t_alpha_beta: float | None = None,
        diff_amp: float = 0.0,
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                if self.use_softplus:
                    de_nom = softplus(
                        de_nom, beta=self.beta_softplus, threshold=self.threshold_softplus if self.threshold_softplus != 0 else current_eps
                    )
                ...
                if not self.rectify_variance or step_size > 0 or n_sma >= self.n_sma_threshold:
                    if self.weight_decouple:
                        ...
                        p_fp32.mul_(
                            1.0
                            - (1.0 if self.fixed_decay else step_size if not self.lr_decouple else step_size / self.max_lr)
                            * self.weight_decay
                            * (1.0 / ema_squared_normalized if self.stable_decay else 1.0)
                        )
                ...
                if (
                    (not self.rectify_variance or step_size > 0 or n_sma >= self.n_sma_threshold)
                    and self.weight_decay > 0.0
                    and self.use_slow_ema
                    and not self.weight_decouple
                ):
    ...
    def reset(self):
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                factored_dims = create_factored_dims(p.shape, factored=group["factor_second_moment"], min_dim_size_to_factor=32)
                ...
                if factored_dims is not None:
                    ...
                    state["exp_avg_sq"] = [
                        torch.zeros(row_shape, dtype=torch.float32, device=p.device).detach(),
    ...
    def step(self, closure: Closure = None) -> Loss:
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                if len(state) == 0:
                    state["exp_avg"] = torch.zeros_like(p)
                    factored_dims = create_factored_dims(grad.shape, factored=group["factor_second_moment"], min_dim_size_to_factor=32)
                ...
                if use_stable_spam_clipping:
                    if group["torch_compile"]:
                        grad = stable_spam_clipping_compile_wrapper(state, grad, step=group["step"], scale=scale)
                    else:
                        grad = stable_spam_clipping_impl(state, grad, step=group["step"], scale=scale)
