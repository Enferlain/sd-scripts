...
class BlockPartitioner:
    ...
    def __init__(self, var: torch.Tensor, rank: int, block_size: int, pre_conditioner_type: int):
        ...
        self.pre_conditioner_shapes: list[list[torch.Tensor]] = self.build_pre_conditioner_shapes(split_sizes, pre_conditioner_type, rank)

    @staticmethod
    def build_pre_conditioner_shapes(split_sizes: list[torch.Tensor], pre_conditioner_type: int, rank: int) -> list[list[torch.Tensor]]:
    ...
    def merge_partitions(self, partitions: list[torch.Tensor]) -> torch.Tensor:
        ...
        for i, indices in reversed(self.splits):
            ...
            partitions = [torch.cat(partitions[idx : idx + n], dim=i) for idx in range(0, len(partitions), n)]  # fmt: skip
...
class PreConditioner:
    ...
    def __init__(
        ...
        self.transformed_shape: list[int] = merge_small_dims(self.original_shape, block_size) if shape_interpretation else list(var.shape)

        self.should_precondition_dims: list[bool] = self.get_should_precondition_dims()
        self.rank: int = sum(self.should_precondition_dims)
        self.exponent_for_pre_conditioner: int = self.inverse_exponent_override if self.inverse_exponent_override > 0 else 2 * self.rank
        ...
        if len(self.transformed_shape) > 1 and not self.skip_precondition(var):
            ...
            shapes: list[list[torch.Tensor] | None] = self.partitioner.shapes_for_pre_conditioners()
    ...
    def compute_pre_conditioners(self) -> None:
        ...
        for i, statistic in enumerate(self.statistics):
            self.pre_conditioners[i] = (
                ...
                if self.use_svd
                else compute_power_schur_newton(mat_g=statistic, p=self.exponent_for_pre_conditioner, ridge_epsilon=self.matrix_eps)
            )
...
def matrix_eigenvalue_decomposition(
    ...
    try:
        ...
        eigenvalues, eigenvectors = torch.linalg.eigh(a_matrix)
    except Exception as exception:
        # If the computation fails and retry_double_precision is True, retry in double precision
        if retry_double_precision and a_matrix.dtype != torch.float64:
            print(
                f"Failed to compute eigendecomposition in {a_matrix.dtype} precision with exception {exception}! "
                "Retrying in double precision..."
            )
            eigenvalues, eigenvectors = torch.linalg.eigh(a_matrix.double())
        else:
