...
class ADOPT(BaseOptimizer):
    ...
    @staticmethod
    def get_rms(value: torch.Tensor) -> float:
        r"""Get RMS."""
        ...
        for group in self.param_groups:
            ...
            for parameter in group["params"]:
                ...
                if group["weight_decay"] != 0 and not group["weight_decouple"]:
                    grad = grad.add(parameter_fp32, alpha=group["weight_decay"])
                ...
                if group["weight_decay"] != 0 and group["weight_decouple"]:
                    parameter_fp32.add_(parameter_fp32, alpha=-group["lr"] * group["weight_decay"])

                denominator = exp_avg_sq.sqrt().add_(group["eps"])
                normalized_grad = grad.div(denominator)
                if group["clip"] is not None:
                    clip = (group["step"] - 1) ** group["clip"]
                    normalized_grad.clamp_(-clip, clip)
