        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                if group["centralization"] > 0.0 and grad.dim() > 1:
                    grad.sub_(grad.mean(dim=tuple(range(1, grad.dim())), keepdim=True).mul_(group["centralization"]))
            ...
            for p in group["params"]:
                ...
                if not group["rectify_variance"] or step_size > 0 or n_sma >= group["n_sma_threshold"]:
                    if group["weight_decouple"]:
                        p_fp32.mul_(
                            1.0
                            - (1.0 if group["fixed_decay"] else step_size)
                            * group["weight_decay"]
                            / (exp_avg_sq_mean if group["stable_decay"] else 1.0)
                        )
...
class RMSPropADOPT(BaseOptimizer):
    ...
    Arguments:
        ...
        adaptive_clip_type (string):
            The type of clipping, can be unit or layer.
        factor_second_moment (bool):
            Stores the second moment at the row/column level instead of per parameter, saving VRAM at the cost of lower precision.
        debias_beta (bool):
    ...
    def step(self, closure: Closure = None) -> Loss:
        ...
        for group in self.param_groups:
            ...
            bias_correction_sqrt: float = math.sqrt(self.debias(beta, group["step"])) if group["debias_beta"] else 1.0
            ...
            for p in group["params"]:
                ...
                if group["weight_decay"] != 0 and group["weight_decouple"] and group["stable_weight_decay"] and param_size > 0:
...
class RMSPropADOPTMARS(BaseOptimizer):
    r"""ADOPT Style RMSProp with MARS-style gradient correction."""
    ...
    def step(self, closure: Closure = None) -> Loss:
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                grad_diff.add_(grad)
                correction = group["gamma"] * grad_diff
                c_t = grad + correction
