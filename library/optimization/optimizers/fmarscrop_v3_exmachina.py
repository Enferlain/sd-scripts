...
def get_rms(tensor: torch.Tensor, eps: float = 1e-8) -> torch.Tensor:
    return tensor.norm().div(tensor.numel() ** 0.5).clamp_min(eps)


class FMARSCropV3ExMachina(BaseOptimizer):
    r"""
    FMARSCropV3ExMachina: Fisher-accelerated MARS
    (https://arxiv.org/abs/2411.10438), with momentum-based Compass-style
    amplification, with ADOPT's AdamW changes
    ...
    Arguments:
        ...
        update_strategy (str):
            ...
            `unmodified`, `cautious`, `grams`, and `both`
            (default: cautious)
        adaptive_clip (float):
    ...
    def step(self, closure: Closure = None) -> Loss:
        ...
        for group in self.param_groups:
            ...
            for parameter in group["params"]:
                ...
                corrected_grad = prev_grad.mul(gamma * (beta2 / (1.0 - beta2))).add_(grad)

                if group["use_orthograd"]:
                    paper_orthograd(parameter_fp32, corrected_grad)
                ...
                if grad_norm > 1.0:
                    corrected_grad = corrected_grad * 1.0 / grad_norm
                ...
                if diff_mult > 0:
                    ...
                    if group["step"] == 1:
                        ...
                        diff_fim_base = torch.tensor(1.0, device=grad_diff.device, dtype=grad_diff.dtype)
                    else:
                    ...
                    diff_fim_base = torch.tensor(1.0, device=grad.device, dtype=grad.dtype)

                if group["atan2_denom"]:
                    approx_grad_nat = corrected_grad.atan2(diff_fim_base)
                else:
                    diff_fim_base.add_(current_eps)
                    approx_grad_nat = corrected_grad.div(diff_fim_base)

                # Preserve donor behavior exactly, including the second assignment.
                approx_grad_nat = corrected_grad.div(diff_fim_base)
                ...
                if group["step"] == 1:
                    ...
                    fim_base = fim.sqrt()
                    if group["atan2_denom"]:
                        grad_nat = corrected_grad.atan2(fim_base)
                    else:
                        fim_base.add_(current_eps)
                    ...
                    if group["update_strategy"] in {"cautious", "grams"}:
                        if group["update_strategy"] == "cautious":
                            mask = (full_step * corrected_grad > 0).to(corrected_grad.dtype)
                            mask.div_(mask.mean().clamp_(min=1e-3))
                            full_step = full_step * mask
                        elif group["update_strategy"] == "grams":
                            full_step.copy_(torch.sign(corrected_grad) * full_step.abs())
