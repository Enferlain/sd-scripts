...
class TrainingDataset(IterableDataset):
    ...
    def __init__(
        ...
        latent_cache_backend: CacheBackend,
        te_cache_backend: CacheBackend | None = None,
        ...
        Args:
            ...
            latent_cache_backend: CacheBackend for loading latent caches.
            te_cache_backend: Optional CacheBackend for loading text encoder caches.
        ...
        self.latent_cache_backend = latent_cache_backend
        self.te_cache_backend = te_cache_backend
    ...
    def _load_batch(
        ...
        for entry in entries:
            if entry.latent_cache_path:
                cache_data = self.latent_cache_backend.load_cache(Path(entry.latent_cache_path))
        ...
        elif self.te_cache_backend and entries[0].te_cache_path:
    ...
    def _load_te_outputs(self, entries: list[CacheEntry]) -> dict[str, torch.Tensor]:
        ...
        assert self.te_cache_backend is not None

        outputs = []
        for entry in entries:
            if entry.te_cache_path:
                cache_data = self.te_cache_backend.load_cache(Path(entry.te_cache_path))
...
def create_training_dataloader(
    ...
    Args:
        ...
        latent_cache_backend: CacheBackend for loading latents.
        te_cache_backend: Optional CacheBackend for TE outputs (cached per-image).
    ...
    dataset = TrainingDataset(
        ...
        latent_cache_backend=latent_cache_backend,
        te_cache_backend=te_cache_backend,
