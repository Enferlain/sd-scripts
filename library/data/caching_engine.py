...
class CachingEngine:
    ...
    Delegates model-specific encoding to a CacheBackend.
    """
    ...
    def __init__(
        ...
        Args:
            backend: Model-specific cache backend.
    ...
    def _get_entries_to_cache(
        ...
        for entry in manifest.entries.values():
            ...
            cache_path_str = self.backend.get_entry_cache_path(entry)
            ...
            if skip_existing and cache_path.exists():
                ...
                try:
                    if self.backend.is_cache_valid(cache_path, entry, flip_aug, alpha_mask):
                        continue
                    else:
    ...
    def _cache_batch(
        ...
        for img, entry in zip(images, entries):
            tensor = self.backend.preprocess_image(
                img,
        ...
        # Encode via strategy
        encoded_list = self.backend.encode_batch(batch_tensor, model, entries)
        ...
        for entry, encoded in zip(entries, encoded_list):
            ...
            self.backend.save_cache(encoded, cache_path)
