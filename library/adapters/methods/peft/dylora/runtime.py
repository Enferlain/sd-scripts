...
def _build_dylora_target_name(target) -> str:
    return f"dylora_{target.path}".replace(".", "_")
...
class DyloraAdapterRuntime(nn.Module):
    """Repo-owned runtime for absorbed DyLoRA modules."""

    def __init__(self, modules: list[DyloraModule]):
        super().__init__()
    ...
    def load_weights(self, file: str):
        state_dict = load_dylora_state_dict(file)
    ...
    def save_weights(self, file: str, dtype, metadata: dict[str, str] | None):
        save_dylora_state_dict(self.dylora_modules, file, dtype=dtype, metadata=metadata)


def _attach_trainable_ref_provider(adapter: DyloraAdapterRuntime, request: AdapterBuildRequest) -> DyloraAdapterRuntime:
    def describe_trainable_parameter_refs() -> list[AdapterTrainableParameterRef]:
        refs: list[AdapterTrainableParameterRef] = []
        for module in adapter.dylora_modules:
...
def _create_dylora_module_from_target(target, request: AdapterBuildRequest) -> DyloraModule:
    ...
    block_size = settings.pop("block_size", 1)
    if adapter_rank is None:
        raise ValueError("DyLoRA runtime settings must include adapter_rank; set adapter.peft.dylora.rank explicitly.")
    ...
    if settings:
        ...
        raise ValueError(f"DyLoRA runtime received unsupported settings: {unknown}.")

    module = DyloraModule.from_target_module(_build_dylora_target_name(target), target.module, config=config)
    module.adapter_target = target
    return module


def create_adapter(request: AdapterBuildRequest):
    """Create a repo-owned DyLoRA runtime from resolved adapter targets."""

    supported_targets = _iter_supported_targets(request.resolved_targets)
    if not supported_targets:
        raise ValueError("DyLoRA adapter runtime requires at least one supported resolved target module")

    adapter = DyloraAdapterRuntime([_create_dylora_module_from_target(target, request) for target in supported_targets])
...
def create_adapter_from_weights(request: AdapterBuildRequest, weights_path: str):
    """Create a loaded repo-owned DyLoRA runtime from saved weights."""

    weights_sd = load_dylora_state_dict(weights_path)
    modules: list[DyloraModule] = []
    for target in _iter_supported_targets(request.resolved_targets):
        lora_name = _build_dylora_target_name(target)
        if not DyloraModule.algo_check(weights_sd, lora_name):
            continue
        up, down, alpha, block_size = DyloraModule.extract_state_dict(weights_sd, lora_name)
        if any(param is None for param in (up, down, alpha)):
            continue
        assert up is not None
        assert down is not None
        assert alpha is not None
        module = DyloraModule.make_module_from_state_dict(
            lora_name,
            target.module,
            up,
            down,
    ...
    if not modules:
        raise ValueError(f"No DyLoRA weights in '{weights_path}' matched the resolved adapter targets")

    adapter = DyloraAdapterRuntime(modules)
    adapter.adapter_resolved_targets = request.resolved_targets
