...
class TrainingConfig:
    ...
    clip_skip: int | None = field(default=None, metadata={"help": "Skip last N CLIP layers (None uses full output, 2 common for anime)"})
    dry_run: bool = field(default=False, metadata={"help": "Run a dry run of the training process"})
    initial_epoch: int | None = field(default=None, metadata={"help": "Initial epoch number when resuming training"})
    initial_step: int | None = field(default=None, metadata={"help": "Initial step number when resuming (includes all epochs)"})
