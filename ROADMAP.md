...
- [ ] **Dashboard / logging system rework** — The first-pass training observability refactor now lives under `library/logging/console.py`, `metrics.py`, `summaries.py`, `reports.py`, and `resource_monitor/`, but the future UI/dashboard side still needs its own broader product/system design. Keep that follow-up focused on new sinks/backends and UI behavior rather than reopening the console/report/resource ownership boundaries that now exist.
  ...
  - The governing resource-system design remains `openspec/changes/resource-intelligence-system/` until final verification and archival. Settled ownership and compatibility boundaries are documented in `docs/resource_intelligence.md`.
...
- [ ] **Resource intelligence and smarter resource management** - The governing `resource-intelligence-system` implementation now has bounded observation-frame ingestion, metadata-backed resource-run queries, durable profiles, evidence-constrained accounting, projected artifacts, and failure-safe cleanup. Final test execution/archival remains before the change closes; later work should build training/inference decisions such as safe tiled-VAE fallback on this settled fact model.
