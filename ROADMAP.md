...
- [ ] **Strategy system follow-up** — Phase 1 shared token/TE shape leaks, the base `TrainingStrategy` facet split, and the extraction of clearly generic training mechanics out of `base/training.py` are now done; remaining cleanup is naming/organization in `base/training.py`, trimming legacy/transitional surface, removing generic-base CLIP assumptions, and removing active singleton strategy dependence. See `docs_design/strategy_system_followup.md`.
