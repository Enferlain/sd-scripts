...
- [ ] **Dashboard / logging system rework** — The first-pass training observability refactor now lives under `library/logging/console.py`, `metrics.py`, `summaries.py`, `reports.py`, and `resource_monitor/`, but the future UI/dashboard side still needs its own broader product/system design. Keep that follow-up focused on new sinks/backends and UI behavior rather than reopening the console/report/resource ownership boundaries that now exist.
  ...
  - Benchmark reports now consume metadata-backed resource views with durable profiles, evidence-constrained accounting statements, and explicit gaps; follow-up should improve explanations or UI behavior rather than add a parallel summary path for the same facts.
  - The completed resource-system requirements now live under `openspec/specs/resource-intelligence/`, `resource-visibility/`, and `training-observability/`; settled ownership and compatibility boundaries are documented in `docs/resource_intelligence.md`.
...
- [x] **Resource intelligence foundation** - The completed `resource-intelligence-system` implementation has bounded observation-frame ingestion, metadata-backed resource-run queries, durable profiles, evidence-constrained accounting, projected artifacts, and failure-safe cleanup. Basic and sampled training runs plus focused failure/OOM regression tests verified the settled fact model before archival.
- [ ] **Smarter resource management** - Build config-bound guidance, regression baselines, capacity recommendations, and training/inference decisions such as safe tiled-VAE fallback from accepted resource profiles and accounting evidence rather than adding a parallel telemetry system.
...
- [ ] Metadata system follow-ups — the backbone, observability/resource, and model-family slices now use the shared typed runtime path. Remaining major slices include data/cache emitters (`sd-scripts-ucs`), adapter-method metadata providers (`sd-scripts-r61`), broader optimization provenance, and analytics/run-warehouse work.
  ...
  - [ ] Stable source-model identity and run provenance (`sd-scripts-8ck`) — represent supplied and resolved model sources as typed accepted facts distinct from run-scoped realizations and produced artifacts; link each run realization to its actual base/component sources with sufficient revision or content evidence to make cross-run equality explicit rather than inferred from a filename.
  - [ ] Typed metadata query capability with optional source resolution (`sd-scripts-edl`) — research consumer-initiated, identity-based queries over already accepted facts and the conditions under which a missing fact may be obtained from an explicit live or artifact source. Live objects remain necessary for operations and mutable observations, while valid accepted entity facts should be reusable by execution and non-execution consumers. Keep querying independent from implicit filing/output; reuse policy and invalidation remain a later seam. This system-wide design must settle before granular model metadata is completed.
  - [ ] Queryable model structure/tensor catalog (`sd-scripts-b25`, blocked by `sd-scripts-8ck`, `sd-scripts-edl`, and `sd-scripts-d79`) — extend the completed top-level model realization/component slice with qualified modules, parameters, buffers, structural tensor descriptors, filtered queries, component aggregates, and optional complete inventories. Reuse it for inspection, resource summaries, optimization diagnostics, and future intelligent systems while keeping routine recording bounded and physical runtime placement distinct from logical structure.
