...
- [ ] **Adapter-system follow-up** — The active adapter rework now has optimization-owned trainable/grouping handoff, a first nested `peft.lora` method surface as the live LoRA config authority under the existing PEFT shell, and an explicit persistence split where `PeftMode` orchestrates checkpoint/export flows while adapter runtime objects participate through repo-owned persistence helpers. Remaining work in the active `adapter-system-rework` change is the broader built-in-path migration/validation slice, not more compatibility-owned optimizer or persistence wiring.
