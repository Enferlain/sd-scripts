...
- [ ] **Dashboard / logging system rework** — The first-pass training observability refactor now lives under `library/logging/console.py`, `metrics.py`, `summaries.py`, `reports.py`, and `resource_monitor.py`, but the future UI/dashboard side still needs its own broader product/system design. Keep that follow-up focused on new sinks/backends and UI behavior rather than reopening the console/report/resource ownership boundaries that now exist.
  ...
  - The governing resource-system design is `openspec/changes/resource-intelligence-system/`: observations, structural facts, durable profiles, evidence-constrained accounting, metadata-backed queries, and projected artifacts must evolve through that one model rather than separate monitoring, provenance, or phase-accounting tracks.
...
- [ ] **Resource intelligence and smarter resource management** - Continue the governing `resource-intelligence-system` OpenSpec toward bounded observation-frame ingestion, resource-run queries, durable profiles, evidence-constrained accounting, and later training/inference decisions such as safe tiled-VAE fallback.
